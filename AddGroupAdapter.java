...
import com.example.baselife.AddGroupActivity;
...
public class AddGroupAdapter extends RecyclerView.Adapter<AddGroupAdapter.MyViewHolder> {
    ...
    public class MyViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public MyViewHolder(View view, Activity mActivity) {
            ...
            activity = mActivity;
            subGroupName = (TextView) view.findViewById(R.id.tvsubgroup);
        }

        @Override
        public void onClick(View view) {
            mySelectedGroup = groupList.get(this.getPosition());
            AddGroupActivity.layout1.setVisibility(View.GONE);
            AddGroupActivity.layout2.setVisibility(View.VISIBLE);
            AddGroupActivity.searchresulttext1.setText(mySelectedGroup.getName());
            AddGroupActivity.searchresulttext2.setText(mySelectedGroup.getName());
        }
    }


    public AddGroupAdapter(List<GroupModel> groupList) {
    ...
    public void onBindViewHolder(MyViewHolder holder, int position) {
        ...
        for (int i = 0; i < groupList.size(); i++) {
            if (groupList.get(i).getId() == groupList.get(position).getParentGroup()) {
                holder.subGroupName.setText(groupList.get(i).getName());
                break;
            }
        }
    }
